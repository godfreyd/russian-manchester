...
if ($bAllowAccess)
{
	...
	$arResult["PROPERTY_LIST_CODE"] = array();
	$arResult["PROPERTY_CODE_ID"] = array();
	...
	foreach ($arResult["PROPERTY_LIST_FULL"] as $key => $arr)
	{
		if (in_array($key, $arParams["PROPERTY_CODES"])) {
            $i = array_search($key, $arParams["PROPERTY_CODES"]);
            $arResult["PROPERTY_LIST"][$i] = $key;
            $arResult["PROPERTY_LIST_CODE"][$i] = $key;
            $arResult["PROPERTY_CODE_ID"][$key] = $key;
        }
	}
	...
	while ($arProperty = $rsIBLockPropertyList->GetNext())
	{
		...
		if (in_array($arProperty["ID"], $arParams["PROPERTY_CODES"])) {
            $i = array_search($arProperty["ID"], $arParams["PROPERTY_CODES"]);
			$arResult["PROPERTY_LIST"][$i] = $arProperty["ID"];
			$arResult["PROPERTY_LIST_CODE"][$i] = "PROPERTY_" . $arProperty["CODE"];
			$arResult["PROPERTY_CODE_ID"]["PROPERTY_" . $arProperty["CODE"]] = $arProperty["ID"];
        } elseif (in_array("PROPERTY_" . $arProperty["CODE"], $arParams["PROPERTY_CODES"])) {
			$i = array_search("PROPERTY_" . $arProperty["CODE"], $arParams["PROPERTY_CODES"]);
			$arResult["PROPERTY_LIST"][$i] = $arProperty["ID"];
			$arResult["PROPERTY_LIST_CODE"][$i] = "PROPERTY_" . $arProperty["CODE"];
            $arResult["PROPERTY_CODE_ID"]["PROPERTY_" . $arProperty["CODE"]] = $arProperty["ID"];
        }
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		foreach ($arParams["PROPERTY_CODES"]  as $i => $propertyID)
		{
            ...
            $propertyID = $arResult['PROPERTY_CODE_ID'][$propertyCode];
				...
				if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "F")
				{
					...
					if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y")
					{
						...
						if (!is_array($arPropertyValue))
						{
                            ...
                            if (strlen($arPropertyValue) > 0 && $propertyCode == "PROPERTY_EMAIL") {
                                if (!check_email($arPropertyValue)) {
                                    $arResult["ERRORS"][] = GetMessage("IBLOCK_ADD_EMAIL_ERROR");
                                    $arResult["ERRORS_FIELDS"][] = $propertyCode;
                                }
                            }
						}
						else
						{
							foreach ($arPropertyValue as $key => $value)
							{
								...
								)
								{
                                    ...
                                    if (strlen($value) > 0 && $propertyCode == "PROPERTY_EMAIL") {
					...
					else
					{
						if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "L")
							...
							$arUpdatePropertyValues[$propertyID] = $arPropertyValue;
                        if (strlen($arUpdatePropertyValues[$propertyID]) > 0 && $propertyCode == "PROPERTY_EMAIL") {
                            if (!check_email($arUpdatePropertyValues[$propertyID])) {
                                $arResult["ERRORS"][] = GetMessage("IBLOCK_ADD_EMAIL_ERROR");
                                $arResult["ERRORS_FIELDS"][] = $propertyCode;
                            }
                        }
		...
		if ($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0)
		{
			if (!$APPLICATION->CaptchaCheckCode($_REQUEST["captcha_word"], $_REQUEST["captcha_sid"]))
			{
                ...
                $arResult["ERRORS_FIELDS"][] = "CAPTCHA";
			}
		}
                ...
                if (!$arParams["ID"] = $oElement->Add($arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]))
				{
                    ...
                    if ($arParams["EVENT_NAME"]) {
                        ...
                        foreach ($arFields["PROPERTY_VALUES"] as $propId => $propValue) {
                            $propCode = array_search($propId, $arResult["PROPERTY_CODE_ID"]);
                            $arFields[$propCode] = $propValue;
                        }
                        CModule::IncludeModule('bitlate.proshop');
                        $arFields["SETTING_EMAIL_TO"] = COption::GetOptionString("bitlate.proshop", 'NL_SERVICE_ORDER_EMAIL', '#DEFAULT_EMAIL_FROM#', SITE_ID);
	...
	if ($bShowForm)
	{
		...
		$arResult["MESSAGE"] = $arParams["USER_MESSAGE_".$_REQUEST["strIMessage"]];
