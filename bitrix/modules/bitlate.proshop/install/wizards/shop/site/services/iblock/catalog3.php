...
if ($IBLOCK_CATALOG_ID)
{
	...
	if ($IBLOCK_OFFERS_ID)
	{
		...
		$dbRes = CCatalogProduct::GetList(array(), array("IBLOCK_ID" => $IBLOCK_OFFERS_ID), false, false, array());
		while ($arRes = $dbRes->Fetch()) {
			CCatalogProduct::Update($arRes['ID'], array('QUANTITY_RESERVED' => 0));
		}
    ...
    $dbPriceType = CCatalogGroup::GetList(array("SORT" => "ASC"), array("XML_ID" => "BASE"));
    $priceId = 0;
    ...
    if ($priceId > 0) {
        $arFields = array(
            "USER_LANG" => array(
                "ru" => GetMessage("NL_CATALOG_PRICE_NAME_BASE"),
            )
        );
        CCatalogGroup::Update($priceId, $arFields);
    }
    
    $dbPriceType = CCatalogGroup::GetList(array("SORT" => "ASC"), array("XML_ID" => "WHOLESALE"));
    $priceId = 0;
    if ($arPriceType = $dbPriceType->Fetch()) {
        $priceId = $arPriceType['ID'];
    }
    if ($priceId > 0) {
        $arFields = array(
            "USER_LANG" => array(
                "ru" => GetMessage("NL_CATALOG_PRICE_NAME_WHOLESALE"),
            )
        );
        CCatalogGroup::Update($priceId, $arFields);
    }
    
    COption::SetOptionString("bitlate.proshop", "NL_CATALOG_ID", $IBLOCK_CATALOG_ID, false, WIZARD_SITE_ID);
    COption::SetOptionString("bitlate.proshop", "NL_CATALOG_OFFERS_ID", $IBLOCK_OFFERS_ID, false, WIZARD_SITE_ID);
