...
<?foreach ($arResult['ITEMS'] as $key => $arItem) {
    ...
    <div class="item column text-center hover-elements" id="<? echo $strMainID; ?>">
        ...
        <div class="hover-show bx_catalog_item_controls">
            ...
            <?if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])) {?>
                <?if ($discount > 0 && 'Y' == $arParams['SHOW_OLD_PRICE'] && count($arItem['PRICES']) <= 1):?>
                ...
                if ('Y' == $arParams['SHOW_OLD_PRICE'] && $discount > 0) {
                    $arJSParams['PRODUCT']['BASIS_PRICE']['VALUE'] = $maxPriceValue / $arItem['CATALOG_MEASURE_RATIO'];
                    $arJSParams['PRODUCT']['BASIS_PRICE']['ECONOMY'] = $discount / $arItem['CATALOG_MEASURE_RATIO'];
                }
                $arJSParams['PRODUCT']['BASIS_PRICE']['DISCOUNT_VALUE'] = $arJSParams['PRODUCT']['BASIS_PRICE']['DISCOUNT_VALUE'] / $arItem['CATALOG_MEASURE_RATIO'];
                if (count($arItem['PRICES']) > 1) {
                    $iPrice = 0;
                    foreach($arItem['PRICES'] as $arPrice) {
                ...
                <div class="bx_catalog_item_scu" id="<? echo $arItemIDs['PROP_DIV']; ?>">
                    <?if (!empty($arItem['OFFERS']) && isset($arSkuTemplate[$arItem['IBLOCK_ID']])) {
                        ...
                        foreach ($arItem['JS_OFFERS'] as $keyOffer => $arJSOffer) {
                            ...
                            $arItem['JS_OFFERS'][$keyOffer]['PRICE']['DISCOUNT_VALUE'] = $arItem['JS_OFFERS'][$keyOffer]['PRICE']['DISCOUNT_VALUE'] / $arItem['OFFERS'][$keyOffer]['CATALOG_MEASURE_RATIO'];
                            $arItem['JS_OFFERS'][$keyOffer]['BASIS_PRICE']['DISCOUNT_VALUE'] = $arItem['JS_OFFERS'][$keyOffer]['BASIS_PRICE']['DISCOUNT_VALUE'] / $arItem['OFFERS'][$keyOffer]['CATALOG_MEASURE_RATIO'];
