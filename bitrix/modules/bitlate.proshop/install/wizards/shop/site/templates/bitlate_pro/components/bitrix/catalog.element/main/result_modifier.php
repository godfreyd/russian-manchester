...
$propertyVideoCode = $arParams['ADD_VIDEO_PROP'];
$propertyOfferVideoCode = $arParams['OFFER_ADD_VIDEO_PROP'];
if ($propertyVideoCode != '') {
    if (!CModule::IncludeModule('bitlate.proshop')) return;
    $videoValues = array();
    if (
        isset($arResult['DISPLAY_PROPERTIES'][$propertyVideoCode]['VALUE']) &&
        !empty($arResult['DISPLAY_PROPERTIES'][$propertyVideoCode]['VALUE'])
    ) {
        $videoValues = $arResult['DISPLAY_PROPERTIES'][$propertyVideoCode];
    } elseif (
        isset($arResult['PROPERTIES'][$propertyVideoCode]['VALUE']) &&
        !empty($arResult['PROPERTIES'][$propertyVideoCode]['VALUE'])
    ) {
        $videoValues = $arResult['PROPERTIES'][$propertyVideoCode];
    }
    if (count($videoValues) > 0) {
        foreach ($videoValues['VALUE'] as $k => $oneVideoValue) {
            if ($url = BitlateProUtils::getVideoUrl($oneVideoValue)) {
                $productSlider[] = array(
                    'ID' => $videoValues['PROPERTY_VALUE_ID'][$k],
                    'SRC' => $url,
                    'WIDTH' => 0,
                    'HEIGHT' => 0,
                    'TYPE' => 'video',
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
        ...
        if ($propertyOfferVideoCode != '') {
            if (!CModule::IncludeModule('bitlate.proshop')) return;
            $videoValues = array();
            if (
                isset($arOffer['DISPLAY_PROPERTIES'][$propertyOfferVideoCode]['VALUE']) &&
                !empty($arOffer['DISPLAY_PROPERTIES'][$propertyOfferVideoCode]['VALUE'])
            ) {
                $videoValues = $arOffer['DISPLAY_PROPERTIES'][$propertyOfferVideoCode];
            } elseif (
                isset($arOffer['PROPERTIES'][$propertyOfferVideoCode]['VALUE']) &&
                !empty($arOffer['PROPERTIES'][$propertyOfferVideoCode]['VALUE'])
            ) {
                $videoValues = $arOffer['PROPERTIES'][$propertyOfferVideoCode];
            }
            if (count($videoValues['VALUE']) > 0) {
                foreach ($videoValues['VALUE'] as $k => $oneVideoValue) {
                    if ($url = BitlateProUtils::getVideoUrl($oneVideoValue)) {
                        $offerSlider[] = array(
                            'ID' => $videoValues['PROPERTY_VALUE_ID'][$k],
...
$arResult['SHOWED_PROPERTIES'] = array();
foreach(CIBlockSectionPropertyLink::GetArray($arParams['IBLOCK_ID'], $arResult['IBLOCK_SECTION_ID']) as $PID => $arLink)
{
	$arResult['SHOWED_PROPERTIES'][] = $PID;
}

$rating = floatval($arResult['PROPERTIES']['rating']['VALUE']);
$vote_count = intval($arResult['PROPERTIES']['vote_count']['VALUE']);

if (empty($rating))
	$rating = floatval($arResult['PROPERTIES']['DEFAULT_RATING']['VALUE']);

if ($rating > 5)
	$rating = 5;

$arResult['RATING'] = $rating;
$arResult['VOTE_COUNT'] = $vote_count;

$cp = $this->__component;

if (is_object($cp))
{
    $cp->arResult['ACCESSORIES_VALUE'] = $arResult['PROPERTIES']['ACCESSORIES']['VALUE'];
