		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
            ...
            if ('Y' == $arParams['PRODUCT_DISPLAY_MODE'])
			{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
                    ...
                    if (!$arOffer['OWNER_PICT'])
					{
                        ...
                        if ($arOffer['PREVIEW_PICTURE']['ID'] > 0) {
                            $pic = CFile::ResizeImageGet($arOffer["PREVIEW_PICTURE"]['ID'], $arSizePreview, BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);
                        }
                        if ($pic === false) {
                ...
                if ($arItem['PRODUCT_PREVIEW']['ID'] > 0) {
                    $pic = CFile::ResizeImageGet($arItem["PRODUCT_PREVIEW"]['ID'], $arSizePreview, BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);
                }
                if ($pic === false) {
                    $pic['src'] = SITE_TEMPLATE_PATH . "/images/no_photo.png";
                }
        ...
        while($obRes = $res->GetNextElement()) {
            ...
            if ($smallPicture > 0) {
                ...
                if ($size == 'gorizontal_priduct') {
                    ...
                    $picBig = CFile::ResizeImageGet($bigPicture, array('width' => 481, 'height' => 291), BX_RESIZE_IMAGE_EXACT, true);
                }
                ...
                if ($arRes['PROPERTY_POSITION_VALUE'] < $bannerPosition) {
                    $bannerPosition++;
                }
            }
        }
