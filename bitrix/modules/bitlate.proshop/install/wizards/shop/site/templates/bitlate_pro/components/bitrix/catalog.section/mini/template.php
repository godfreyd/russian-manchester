...
if (!empty($arResult['ITEMS']))
{
    ...
    $isPriceComposite = COption::GetOptionString("bitlate.proshop", "NL_CATALOG_PRICE_COMPOSITE", false, SITE_ID);?>
    ...
    <div class="product-list-item table-container h100pc" id="<? echo $strMainID; ?>">
        ...
        <div class="table-item item-info small-9">
            <div class="inline-block-container">
                        ...
                        <div class="inline-block-item vertical-middle small-12 column-info">
                            ...
                            <div class="row">
                                ...
                                <?if ($arParams['SHOW_MAX_QUANTITY']):
                                    if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])):
                                        $quantityInfo = BitlateProUtils::getProductAmount($arItem['CATALOG_QUANTITY'], $arParams['MIN_AMOUNT'], $arParams['MAX_AMOUNT']);
                                        $quantityText = ($arParams['USE_MIN_AMOUNT'] == "Y") ? $quantityInfo['text'] : $quantityInfo['products'];?>
                                        <div class="column column-info-block small-6 medium-4 large-4 xxlarge-3 existence <?=$quantityInfo['class']?>" title="<?=$quantityText?>">
                                ...
                                <?if (isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])):?>
                                    <?foreach ($arItem['OFFERS'] as $key => $arOffer):
                                        $strVisible = ($key == $arItem['OFFERS_SELECTED'] ? '' : 'none');?>
                                        <div class="column column-info-block small-6 medium-4 large-3 xxlarge-4 float-left" id="<? echo $arItemIDs['LIKED_COMPARE_ID']; ?><?=$arOffer['ID']?>" style="display: <? echo $strVisible; ?>;">
                        ...
                        <div class="inline-block-item vertical-middle relative column-action">
                            <div id="<? echo $arItemIDs['PRICE']; ?>_block">
                                ...
                                <?endif;?>
                                        ...
                                        <div id="<? echo $arItemIDs['PRICE']; ?>">
                                            <?foreach ($arItem['PRICES'] as $arPrice):
                                                ...
                                                <div class="price-block">
                                                    ...
                                                    <div class="price"><?if (!empty($minPrice)):?><?=CCurrencyLang::CurrencyFormat($arPrice['DISCOUNT_VALUE'], $arPrice['CURRENCY'])?><?if ('Y' == $arParams['SHOW_OLD_PRICE'] && $discountPrice > 0):?> <span class="old"><?=CCurrencyLang::CurrencyFormat($maxItemPriceValue, $arPrice['CURRENCY'])?></span><?endif;?><?endif;?></div>
                                    ...
                                    <?else:?>
                                        <div class="price-block" id="<? echo $arItemIDs['PRICE']; ?>">
                                            <div class="price">
                ...
                <?} else {?>
                    <div class="bx_catalog_item_scu inline-block-item vertical-middle row row-cart large-4 xxlarge-3" id="<? echo $arItemIDs['PROP_DIV']; ?>">
                                ...
                                <?foreach ($arSkuTemplate as $code => $strTemplate) {
                                    if (!isset($arItem['OFFERS_PROP'][$code])) {
                                        ...
                                        if ('TEXT' == $arProp['SHOW_MODE'])
                                        {
                                            ...
                                            $templateRow .= '<div class="inline-block-container show-for-large" id="#ITEM#_prop_'.$arProp['ID'].'_cont" style="display:none;">'.
...
<div class="row collapse catalog-footer">
    ...
    <div class="column large-4 small-12 text-center">
        <?if ($nextPageUrl != ''):?>
            <a href="javascript:;" class="load-more text-center" onclick="getCatalogItems(this, '<?=(($arParams["TAB_TYPE"]) ? '.product-grid-' . $arParams["TAB_TYPE"] . ' .product-grid' : '.product-list.mini')?>', false<?if ($arParams["TAB_TYPE"]):?>, true<?endif;?>)" data-ajax="<?=$nextPageUrl?>">
                <svg class="icon">
