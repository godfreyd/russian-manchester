...
if ($ElementID > 0)
{
	...
	if (!empty($arRecomData))
	{
		...
		{
            $comparePath = $arResult['FOLDER'].$arResult['URL_TEMPLATES']['compare'];
            ...
            ?><?$APPLICATION->IncludeComponent("bitrix:catalog.bigdata.products", "", array(
                    ...
                    'DISPLAY_COMPARE' => (isset($arParams['USE_COMPARE']) ? $arParams['USE_COMPARE'] : ''),
                    'COMPARE_URL_TEMPLATE' => $comparePath.$arParams["ACTION_VARIABLE"]."=ADD_TO_COMPARE_LIST&".$arParams["PRODUCT_ID_VARIABLE"]."=#ID#",
                    'COMPARE_PATH' => $arResult['FOLDER'].$arResult['URL_TEMPLATES']['compare'],
                    'SLIDER_ZINDEX' => 1,
                ),
                $component,
