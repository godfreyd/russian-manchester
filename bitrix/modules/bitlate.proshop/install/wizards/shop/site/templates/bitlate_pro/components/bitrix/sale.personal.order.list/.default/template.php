...
<?if (!empty($arResult['ORDERS']) && is_array($arResult['ORDERS'])):?>
    <ul class="profile-block-list" data-ajax="<?=SITE_DIR?>nl_ajax/history_order.php">
        <?foreach ($arResult['ORDERS'] as $arData):?>
            <li>
                <a href="<?=$arData["ORDER"]["URL_TO_DETAIL"]?>" class="profile-order-<?=$arData['ORDER']['ID']?>"><?=GetMessage('STPOL_ORDER_NO')?><?=$arData['ORDER']['ACCOUNT_NUMBER']?></a> <span class="profile-block-list-date"><?=GetMessage('STPOL_FROM')?> <?=$arData['ORDER']['DATE_INSERT_FORMATED']?></span>
