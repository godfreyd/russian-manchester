...
while ($arSite = $rsSites->Fetch()) {
    $catalogId = COption::GetOptionString("bitlate.proshop", "NL_CATALOG_ID", 0, $arSite["LID"]);
...
$arAllOptions = array(
    ...
    array("NL_CATALOG_VIEW", GetMessage("NL_CATALOG_VIEW_TITLE"), "board", array("selectbox", BitlateProUtils::getViewList())),
    ...
    array("NL_CATALOG_ART_CODE", GetMessage("NL_CATALOG_ART_CODE_TITLE"), false, array("PROPERTY_CODE", $arIBlocksProps, false)),
    array("NL_CATALOG_OFFERS_ART_CODE", GetMessage("NL_CATALOG_OFFERS_ART_CODE_TITLE"), false, array("PROPERTY_CODE_OFFERS", $offersProps, false)),
    array("NL_CATALOG_ADD_PICT_PROP", GetMessage("NL_CATALOG_ADD_PICT_PROP_TITLE"), false, array("PROPERTY_CODE", $arIBlocksProps, false)),
    array("NL_CATALOG_ADD_VIDEO_PROP", GetMessage("NL_CATALOG_ADD_VIDEO_PROP_TITLE"), false, array("PROPERTY_CODE", $arIBlocksProps, false)),
    ...
    array("NL_CATALOG_OFFER_ADD_VIDEO_PROP", GetMessage("NL_CATALOG_OFFER_ADD_VIDEO_PROP_TITLE"), false, array("PROPERTY_CODE_OFFERS", $offersProps, false)),
    ...
    array("NL_MAIN_TABS_PAGE_ELEMENT_COUNT_MAX", GetMessage("NL_MAIN_TABS_PAGE_ELEMENT_COUNT_MAX_TITLE"), "", array("text", 20)),
    array("NL_MAIN_TABS_PAGE_ELEMENT_COUNT_BANNERS", GetMessage("NL_MAIN_TABS_PAGE_ELEMENT_COUNT_BANNERS_TITLE"), "", array("text", 20)),
...
$generateOptions = BitlateProUtils::getGenerateOptions();
$arMultyOptions = BitlateProUtils::getMultyOptions();
...
if ($_SERVER['REQUEST_METHOD'] == "POST" && (strlen($_POST['Update']) > 0 || strlen($_POST['Apply']) > 0) && check_bitrix_sessid())
{
            ...
            if (is_array($arOption)) {
                ...
                if ($errorOption === false) {
                    if (in_array($opName, $generateOptions)) {
                        $oldVal = COption::GetOptionString("bitlate.proshop", $opName, $arOption[1], $siteLID);
                        if ($oldVal != $val) {
                        ...
                        if ($opName == 'NL_CATALOG_ID') {
                            COption::SetOptionString("bitlate.proshop", "NL_CATALOG_OFFERS_ID", $offersInfoIblocks[$val]['ID'], false, $siteLID);
                            COption::SetOptionString("bitlate.proshop", "NL_CATALOG_OFFERS_TYPE", $offersInfoIblocks[$val]['IBLOCK_TYPE_ID'], false, $siteLID);
                        }
                        COption::SetOptionString("bitlate.proshop", $opName, $val, $arOption[1], $siteLID);
                    } else {
                        switch ($opName) {
                                ...
                                if ($oldVal != $val) {
                                    if (CModule::IncludeModule("currency")) {
                                        if ($resCur = CCurrencyLang::GetCurrencyFormat("RUB", "ru")) {
                                            if ($val == "Y") {
                                                COption::SetOptionString("bitlate.proshop", "NL_SHOP_RUB_FORMAT_DEFAULT", $resCur["FORMAT_STRING"], "", $siteLID);
                                                $resCur["FORMAT_STRING"] = COption::GetOptionString("bitlate.proshop", "NL_SHOP_RUB_FORMAT", "", $siteLID);
                                            } else {
                                                $resCur["FORMAT_STRING"] = COption::GetOptionString("bitlate.proshop", "NL_SHOP_RUB_FORMAT_DEFAULT", "", $siteLID);
                                            }
                                            CCurrencyLang::Update("RUB", "ru", array("FORMAT_STRING" => $resCur["FORMAT_STRING"]));
                                            foreach ($arSites as $arSite2) {
                                                $siteLID2 = key($arSite2);
                                                COption::SetOptionString("bitlate.proshop", $opName, $val, $arOption[1], $siteLID2);
                                            }
                                        } else {
    ...
    foreach ($generate as $siteLID => $check) {
        if ($check) {
            BitlateProUtils::generateIncludeFile($siteLID, $arSitesDir[$siteLID]);
        }
    }
}
if (count($arErrors) > 0) {
    ...
    <?foreach($aTabs as $i => $aTab):
        ...
        <?foreach($arAllOptions as $arOption):
            if (is_array($arOption)):
                ...
                $val = COption::GetOptionString("bitlate.proshop", $arOption[0], false, $siteLID);
                $type = $arOption[3];?>
                <tr>
                    ...
                    <td width="60%">
                        ...
                        <?elseif($type[0]=="IBLOCK_ID"):
                            $varArr = explode('|', $val);
                            $typeIblockId = COption::GetOptionString("bitlate.proshop", "NL_CATALOG_TYPE", false, $siteLID);
