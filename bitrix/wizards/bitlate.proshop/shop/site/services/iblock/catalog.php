...
if($useStoreControl == "Y")
{
    ...
    if ($newStoreId) {
        ...
        $fId = CFile::SaveFile($fileArray, "store");
        $fileArray2 = CFile::MakeFileArray(WIZARD_ABSOLUTE_PATH . GetMessage("STORE_PHOTO_1_2"));
        $fId2 = CFile::SaveFile($fileArray2, "store");
        ...
        $GLOBALS["USER_FIELD_MANAGER"]->Update("CAT_STORE", $newStoreId, array('UF_NL_SHOP_PHOTOS' => array($fId, $fId2)));
    }
    if ($newStoreId2) {
        $fileArray = CFile::MakeFileArray(WIZARD_ABSOLUTE_PATH . GetMessage("STORE_PHOTO_2"));
        $fId = CFile::SaveFile($fileArray, "store");
        $fileArray2 = CFile::MakeFileArray(WIZARD_ABSOLUTE_PATH . GetMessage("STORE_PHOTO_2_2"));
        $fId2 = CFile::SaveFile($fileArray2, "store");
        $GLOBALS["USER_FIELD_MANAGER"]->Update("CAT_STORE", $newStoreId2, array('UF_NL_SHOP_METRO' => GetMessage('STORE_METRO_2')));
        $GLOBALS["USER_FIELD_MANAGER"]->Update("CAT_STORE", $newStoreId2, array('UF_NL_SHOP_PHOTOS' => array($fId, $fId2)));
    }
}
...
if (CBXFeatures::IsFeatureEnabled('CatMultiPrice')) {
    ...
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH . "/xml/" . LANGUAGE_ID . "/catalog_base.xml";
