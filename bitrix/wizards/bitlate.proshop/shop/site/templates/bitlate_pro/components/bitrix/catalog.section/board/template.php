...
<?if ($arParams["TEMPLATE_THEME"] != 'slider'):?>
    ...
    <div class="products-flex-grid product-grid<?if (!$arParams["TAB_TYPE"]):?> container<?endif;?><?=$gridClass?>" data-url="<?=$gridUrl?>">
<?endif;?>
<?
    ...
    $arResult['BANNER_POSITION']++;?>
        <div class="item column text-center hover-elements"<?if ($arParams["TEMPLATE_THEME"] == 'slider'):?> id="<? echo $strMainID; ?>"<?endif;?>>
            ...
            <div class="hover-show bx_catalog_item_controls">
                ...
                <?if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])) {?>
                    <?if ($discount > 0 && 'Y' == $arParams['SHOW_OLD_PRICE'] && count($arItem['PRICES']) <= 1 && !(in_array('prodday', $itemType) && $discount > 0 && $arResult['IS_SHOW_LARGE_PREVIEW']) && !(in_array('action', $itemType) && $arResult['IS_SHOW_LARGE_PREVIEW'])):?>
                ...
                <?} else {?>
                    <?if ('Y' == $arParams['SHOW_OLD_PRICE'] && count($arItem['PRICES']) <= 1 && !((in_array('action', $itemType) || in_array('prodday', $itemType)) && $arResult['IS_SHOW_LARGE_PREVIEW'])):?>
                    ...
                    <div class="bx_catalog_item_scu" id="<? echo $arItemIDs['PROP_DIV']; ?>">
                        ...
                        <?if (!empty($arItem['OFFERS_PROP'])) {
                            ...
                            foreach ($arItem['JS_OFFERS'] as $keyOffer => $arJSOffer) {
                                ...
                                if (count($arItem['OFFERS'][$keyOffer]['PRICES']) > 1 && (!$arParams['TAB_TYPE'] || ($arResult['IS_SHOW_LARGE_PREVIEW'] && !in_array('prodday', $itemType) && !in_array('action', $itemType)))) {
                                    $iPrice = 0;
                        ...
                        <?if ('Y' == $arParams['SHOW_OLD_PRICE'] && in_array('action', $itemType) && $arResult['IS_SHOW_LARGE_PREVIEW']):?>
                            </div>
                        <?endif;?>
                        <?unset($canBuy);?>
                    </div>
