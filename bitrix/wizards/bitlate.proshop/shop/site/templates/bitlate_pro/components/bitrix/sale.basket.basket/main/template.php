...
$offersParams = array();
$offersCartProps = COption::GetOptionString("bitlate.proshop", 'NL_CATALOG_CART_OFFERS_PROPERTY_CODE', false, SITE_ID);
            ...
            <form method="post" action="<?=POST_FORM_ACTION_URI?>" name="basket_form" id="basket_form" data-ajax="<?=$APPLICATION->GetCurPage()?>?load=Y">
                <div id="basket_items">
                    <div class="text-center">
                        ...
                        <div class="cart-caption-desc product-price"><?=$normalCount?> <?=BitlateProUtils::nl_inclination($normalCount, GetMessage("SALE_GOODS_1"), GetMessage("SALE_GOODS_2"), GetMessage("SALE_GOODS_10"))?> <?=GetMessage("SALE_ON")?> <?=$arResult["allSum_FORMATED"]?></div>
                    </div>
                    <?foreach ($arResult["GRID"]["ROWS"] as $k => $arItem):
                        if ($arItem["DELAY"] == "N" && $arItem["CAN_BUY"] == "Y"):
                            ...
                            <div class="callout cart-product-item inline-block-container" data-product-id="<?=$arItem["ID"]?>" id="<?=$arItem["ID"]?>">
                                ...
                                <button class="close-button" type="button" onclick="cartAjaxAction('<?=SITE_DIR?>nl_ajax/cart.php', 'delete', '<?=$arItem["ID"]?>')">
