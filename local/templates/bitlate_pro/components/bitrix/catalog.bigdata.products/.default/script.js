			...
			case 3://sku
                ...
                for (i = 0; i < this.offers.length; i++)
                {
                    if (!!this.obBasketActions) {
                        ...
                        BX.bind(buyButton, 'click', BX.delegate(this.Basket, this));
                    }
                    if (this.useCompare)
                    {
...
window.JCCatalogBigdataProducts.prototype.ChangeInfo = function()
{
	...
	if (-1 < index)
	{
		...
		/*if (!!this.obPrice)
		{
			...
			{
				strPrice += ' <span class="old">'+this.offers[index].PRICE.PRINT_VALUE+'</span>';
			}
            ...
            if (!!this.obPriceHover) {
                ...
                if (this.showOldPrice && (this.offers[index].PRICE.DISCOUNT_VALUE !== this.offers[index].PRICE.VALUE))
                {
                    strPriceHover += '<div class="economy">' + this.visual.ECONOMY_HTML.replace('#ECONOMY_PRICE#', BX.Currency.currencyFormat(this.offers[index].PRICE.ECONOMY, this.offers[index].PRICE.CURRENCY, true)) + '</div>';
                }
        ...
        var currentBasisPrice = this.offers[index].BASIS_PRICE;
        
        var calcPrice = {
            ECONOMY: currentBasisPrice.ECONOMY * currentBasisPrice.CATALOG_MEASURE_RATIO,
            DISCOUNT_VALUE: currentBasisPrice.DISCOUNT_VALUE * currentBasisPrice.CATALOG_MEASURE_RATIO,
            VALUE: currentBasisPrice.VALUE * currentBasisPrice.CATALOG_MEASURE_RATIO,
            DISCOUNT_DIFF: currentBasisPrice.DISCOUNT_DIFF * currentBasisPrice.CATALOG_MEASURE_RATIO,
            DISCOUNT_DIFF_PERCENT: currentBasisPrice.DISCOUNT_DIFF_PERCENT,
            CURRENCY: currentBasisPrice.CURRENCY,
            PRICES: [],
        };
        var currentPrices = currentBasisPrice.PRICES;
        if (currentPrices != undefined && currentPrices.length > 1) {
...
window.JCCatalogBigdataProducts.prototype.setPrice = function(price)
{
	var strPrice,
		obData;
...
window.JCCatalogBigdataProducts.prototype.Compare = function()
{
...
window.JCCatalogBigdataProducts.prototype.CompareResult = function(result)
{
...
window.JCCatalogBigdataProducts.prototype.CompareRedirect = function()
{
