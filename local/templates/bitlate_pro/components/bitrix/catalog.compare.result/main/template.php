    ...
    <div class="owl-carousel product-compare-carousel float-right">
        <?foreach($arResult["ITEMS"] as $key => $arCurElement):
            ...
            <div class="item column text-center">
                <div class="item-wrap">
                    <button type="button" class="product-compare-change remove close-button" onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arCurElement['~DELETE_URL'])?>');">
                    ...
                    <?endif;?>
                    <a href="javascript:;"<?if ($arCurElement['CAN_BUY']):?> onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arCurElement["ADD_URL"])?>', '<?=CUtil::JSEscape($arResult["COMPARE_URL_TEMPLATE"])?>', '<?=$arCurElement['ID']?>');"<?endif;?> class="button small add2cart" data-preview="#pic_<?=$arCurElement['ID']?>" data-product-id="<?=$arCurElement['ID']?>"<?if (!$arCurElement['CAN_BUY']):?> disabled="disabled"<?endif;?>><span><?=GetMessage("CATALOG_COMPARE_BUY"); ?></span></a>
...
<script type="text/javascript">
    ...
    <?endif;?>
    <?if ($countCompare > 0):?>
        if ($("#page").hasClass("header-v1")) {
            $('#bx_compare_count .add2compare_counter').html('<?=$countCompare?>');
        } else {
            $('#bx_compare_count .add2compare_counter').html(' (<?=$countCompare?>)');
        }
