...
const RulesModal = ({ doAfterClick }) => {
    ...
    const rules = [1, 2, 3, 4].map(ruleNumber => console.log(`rules-step-${ruleNumber}`)).join('<br>');
        ...
        <Modal type={classNames()}>
            <div className={classNames()}>
                ...
                <div className={classNames('content')}>
                    <div className={classNames('title')}>{console.log('rules-title')}</div>
                    <div className={classNames('rules-wrap')} dangerouslySetInnerHTML={{ __html: rules }} />
                    <Button action onClick={doAfterClick}>{console.log('rules-button')}</Button>
                </div>
